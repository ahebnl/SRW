...
{
    int ncx = round((xEnd - xStart) / xStep + 1);
    int ncz = round((zEnd - zStart) / zStep + 1);
    dest->nx = next_fft_num(ncx + 2 * nxpad);
    dest->nz = next_fft_num(ncz + 2 * nzpad);
    // set padding 0
    nxpad = (dest->nx - ncx) / 2;
    const int nxpad2 = dest->nx - ncx - nxpad;
    nzpad = (dest->nz - ncz) / 2;
    const int nzpad2 = dest->nz - ncz - nzpad;
    ...
    assert(dest->xStep > 1e-10 && dest->zStep > 1e-10); // too crazy small

    fprintf(stderr, "sample inp xStart= %g xEnd= %g xStep= %g ncx= %d\n", xStart, xEnd, xStep, ncx);
    fprintf(stderr, "sample fin xStart= %g xEnd= %g xStep= %g nx= %d\n",
        dest->xStart, dest->xStart + dest->xStep * (dest->nx - 1), dest->xStep, dest->nx);
    
    dest->ReAllocBaseRadAccordingToNeNxNz();

    int64_t SZ = dest->nx * dest->nz * dest->ne * 2LL;
    fprintf(stderr, "sample: padding xL= %d xR= %d nxtotal= %d zL= %d zR= %d nztotal= %d N=%lld\n",
        nxpad, nxpad2, dest->nx, nzpad, nzpad2, dest->nz, SZ);
    ...
    if (nxpad > 0 || nzpad > 0 || nxpad2 > 0 || nzpad2 > 0) {
        for (int iz = 0; iz < dest->nz; ++iz) {
            for (int ix = 0; ix < dest->nx; ++ix) {
                if (iz < nzpad || iz + nzpad2 > dest->nz ||
                    ...
                    int64_t c = iz * dest->nx * dest->ne * 2LL + ix * dest->ne * 2LL;
...
void srTCombinedDrift::init_dest_rad(srTSRWRadStructAccessData& rad, const srTSRWRadStructAccessData* pinrad, int wg) const
{
    ...
    rad.nx = next_fft_num(round(wx * pinrad->xStep * (pinrad->nx - 1) / rad.xStep) + 1);
    rad.nz = next_fft_num(round(wz * pinrad->zStep * (pinrad->nz - 1) / rad.zStep) + 1);
...
int srTCombinedDrift::PropagateRad2(srTSRWRadStructAccessData* pRadAccessData, srTParPrecWfrPropag& ParPrecWfrPropag, srTRadResizeVect& ResBeforeAndAfterVect)
{
    ...
    double xEnd = xStart + xStep * (pRadAccessData->nx - 1);
    double zStep = pRadAccessData->zStep;
    double zStart = pRadAccessData->zStart;
    double zEnd = zStart + zStep * (pRadAccessData->nz - 1);
    const double WX = xEnd - xStart, WZ = zEnd - zStart;
    ...
    cout << "[WARNING!!] The ZP (nzdiv=" << nzdiv << ",nxdiv=" << nxdiv
        ...
        << " xEnd= " << pRadAccessData->xStart + pRadAccessData->xStep * (pRadAccessData->nx - 1)
        << " nx= " << pRadAccessData->nx << endl
        << "      zStart = " << pRadAccessData->zStart << " zStep = " << pRadAccessData->zStep
        << " zEnd= " << pRadAccessData->zStart + pRadAccessData->zStep * (pRadAccessData->nz - 1)
        << " nz= " << pRadAccessData->nz << endl
    ...
    init_dest_rad(destRad, pRadAccessData, 1); // 0 - do not scale with obsgrid yet
    ...
    fprintf(stderr, "dest z in %d [%g %g] step= %g\n", destRad.nz,
        destRad.zStart, destRad.zStart + destRad.zStep * (destRad.nz - 1),
        destRad.zStep);
    fprintf(stderr, "dest x in %d [%g %g] step= %g\n", destRad.nx,
        destRad.xStart, destRad.xStart + destRad.xStep * (destRad.nx - 1),
        destRad.xStep);
    ...
    for (int iz = 0; iz < nzdiv; ++iz) {
        double rz = (iz == 0 ? 0.0 : zdivs[iz-1]);
        double zStart = pRadAccessData->zStart + WZ * rz;
        double zEnd = pRadAccessData->zStart + WZ * zdivs[iz];

        for (int ix = 0; ix < nxdiv; ++ix) {
            double rx = (ix == 0 ? 0.0 : xdivs[ix-1]);
            double xStart = pRadAccessData->xStart + WX * rx;
            double xEnd = pRadAccessData->xStart + WX * xdivs[ix]; 
...
#if DEBUG_ZPD > 1
            fprintf(stderr, "\n\n## split_%d_%d pd= %f %f z [%.4e,%.4e] x [%g %g]\n",
                iz, ix, pdx, pdz, zStart, zEnd, xStart, xEnd);
#endif

            assert(pRadAccessData->zStep / pdz > 1e-10 && pRadAccessData->xStep/pdx > 1e-10); // too crazy small
            ...
            // sample with smaller step size, before passing ZP
            ...
            fprintf(stderr, "## sub_%d_%d pd= %f %f z [%.4e,%.4e] x [%g %g]  (%d,%d)\n", iz, ix, pdx, pdz,
                newRad.zStart, newRad.zStart + (newRad.nz - 1) * newRad.zStep,
                newRad.xStart, newRad.xStart + (newRad.nx - 1) * newRad.xStep,
                newRad.nz, newRad.nx);
            ...
            newRad.dumpBinData(fname, fname, newRad.nz/1000, newRad.nx/1000);
            fprintf(stderr, "zpd10.%d_%d has %lld zero/NaN\n", iz, ix, newRad.checkNanZero());
#endif
            ...
            fprintf(stderr, "zpd11.%d_%d has %lld zero/NaN\n", iz, ix, newRad.checkNanZero());
#endif
...
#if DEBUG_ZPD > 1
            fprintf(stderr, "zpd13.%d_%d has %lld zero/NaN\n", iz, ix, newRad.checkNanZero());
#endif
...
#if DEBUG_ZPD > 1
            {
                ...
                newRad.dumpBinData(fname, fname, newRad.nz / 2000, newRad.nx / 2000);
                {
                ...
                { // rescale only for saving files
                   srTRadResize resz;
                   resz.pxm = resz.pzm = 1.0;
                   resz.pxd = 1200.0 / newRad.nx;
                   resz.pzd = 1200.0 / newRad.nz;
                   // fprintf(stderr, "resize field: resz.pxm= %f, resz.pxd=%f\n", resz.pxm, resz.pxd);
                   srTSRWRadStructAccessData junkRad(newRad);
                   RadResizeGen(junkRad, resz);
                   fname = "junk.zpd2.resz." + to_string(iz) + "_" + to_string(ix) + ".bin";
                   junkRad.dumpBinData(fname, fname); 
                }
            }
#endif
            ...
            resz.pxd = obsgrid[1];
            resz.pzd = obsgrid[3];
            fprintf(stderr, "resize field after drift before accumulation: resz.pxm= %f, resz.pxd=%f\n", resz.pxm, resz.pxd);
            RadResizeGen(newRad, resz);
            fprintf(stderr, "before accum xStart= %g xStep= %g\n", newRad.xStart, newRad.xStep);
            ...
            if (newRad.xStep > 1.02*destRad.xStep || newRad.zStep > 1.02*destRad.zStep) {
...
#if DEBUG_ZPD > 1
            {
                ...
                destRad.dumpBinData(fname, fname, destRad.nz/2000, destRad.nx/2000);
                fname = "junk.zpd2.sum.core3." + to_string(iz) + "_" + to_string(ix) + ".bin";
                destRad.dumpBinDataCore3(fname, fname, 1200, 1200);
            }
#endif
        }
    }
