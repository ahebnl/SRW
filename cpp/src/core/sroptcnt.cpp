	...
	for(int i=0; i<=opt.nElem; i++)
	{
		...
		if(i < opt.nElem)
		{
			...
			else if ((strcmp(sType, "cd") == 0) || (strcmp(sType, "CD") == 0))
			{
				...
				pOptElem = new srTConnectDrift(p->nZones, p->rn, p->thick, p->atLen1, p->atLen2, p->delta1, p->delta2, p->x, p->y, p->e0, p->dftLen, p->nxdiv, p->nzdiv, p->rdivs); //OC22062019
				//pOptElem = new srTZonePlate(p->nZones, p->rn, p->thick, p->atLen1, p->atLen2, p->delta1, p->delta2, p->x, p->y);
			}
...
//int srTCompositeOptElem::PropagateRadiationGuided(srTSRWRadStructAccessData& wfr)
{
	...
	for(srTGenOptElemHndlList::iterator it = GenOptElemList.begin(); it != GenOptElemList.end(); ++it)
	{
		...
		time(&end1);
		double difference1 = difftime(end1, begin);
		printf("Elem %d %.10f seconds is taken for Resize\n", elemCount, difference1);
